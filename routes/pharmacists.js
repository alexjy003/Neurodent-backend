...
router.post('/', uploadSingle('profileImage'), handleUploadResponse, [
  ...
  try {
    ...
    const autoGeneratedPassword = generateSecurePassword(10);
    ...
    try {
      const emailResult = await emailService.sendPharmacistCredentialsEmail(
        email,
        `${firstName} ${lastName}`,
      ...
      if (!emailResult.success) {
        console.error('Failed to send pharmacist credentials email:', emailResult.error);
        // Still return success for pharmacist creation, but note email failure
        return res.status(201).json({
          success: true,
          message: 'Pharmacist added successfully, but failed to send credentials email. Please contact the pharmacist manually.',
          data: {
            pharmacist: {
              id: pharmacist._id,
      ...
      res.status(201).json({
        success: true,
        message: 'Pharmacist added successfully! Login credentials have been sent to their email.',
        data: {
          pharmacist: {
            id: pharmacist._id,
            firstName: pharmacist.firstName,
          ...
          emailSent: true,
          emailPreview: emailResult.previewUrl // For test emails
        }
      });

    } catch (emailError) {
      console.error('Error sending pharmacist credentials email:', emailError);
      res.status(201).json({
        success: true,
        message: 'Pharmacist added successfully, but failed to send credentials email. Please contact the pharmacist manually.',
        data: {
